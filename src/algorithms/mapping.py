...
class MappingField:
    ...
    def registerTile(self, tileCoord: Tuple[int, int], tileType: int = -1, incrementVisitTileCount: int = 0, incrementVisitWallCount: Dict[int, int] = None, wallStatus: Dict[int, int] = None):
            ...
            if wallFieldCoord in self.mapData:
                ...
                if isinstance(existingWallInfo, MappingDataWallInfo):
                     if existingWallInfo.isWall != isWall:
                        print(f"Error: Conflicting wall information at {wallFieldCoord}")
                        return False
            
            self.mapData[wallFieldCoord] = wallInfo
        return True

    def getWallInfo(self, tileCoord: Tuple[int, int]) -> Dict[Direction, Optional[bool]]:
        fieldCoord = self.tileCoord2FieldCoord(*tileCoord)
        wallInfo = {}
        ...
        for direction in Direction:
            ...
            if neighbor in self.mapData and isinstance(self.mapData[neighbor], MappingDataWallInfo):
    ...
    def getTileInfo(self, tileCoord: Tuple[int, int]) -> Optional[MappingDataTileInfo]:
        fieldCoord = self.tileCoord2FieldCoord(*tileCoord)
        if fieldCoord in self.mapData and isinstance(self.mapData[fieldCoord], MappingDataTileInfo):
...
class Mapping:
    ...
    def dijkstra(self, start: Tuple[int, int], startDir: Direction, searchType: str = "all") -> Dict[Tuple[int, int], DijkstraResult]:
        return dijkstra(
            start=start,
            start_dir=startDir,
